...
The migration source remains current at tag `v0.1.13`. Its successful
`build()` path opens generated bytes in a new PkiStudioJS viewer tab by default.
DER Builder intentionally retains that new-tab workflow while replacing the
obsolete viewer with the published DerEditor package and a DER Builder-owned
one-shot transfer format.
...
- Browser I/O is limited to files, clipboard access, downloads, the versioned
  same-origin generated-document transfer owned by DER Builder, and
  DerEditor's public same-origin Send to transfer.
...
6. Open generated DER in a new editable DerEditor tab by default.
7. Use DerEditor's public Send to feature within that tab when another editable
   standalone view is needed.
...
- Log build, parse, validation, DerEditor-tab opening, save, and failure
  operations.
- Keep a successful build result available when browser storage or popup policy
  prevents the DerEditor tab from opening, and report the display failure in
  status and API Log output.
...
- Allow embedders to disable automatic tab opening while still receiving the
  generated document from `build()`.
...
- viewer mounting with an explicit editable state;
...
The DER Builder application owns a separate, versioned one-shot transfer helper
for complete generated documents. That helper may use adapter-provided byte
conversion functions but must not reproduce or consume DerEditor's internal
storage schema.
...
- The normal DER Builder page does not mount a Generated DER viewer.
- A successful build validates the bytes through DerEditor Core and, unless
  disabled by the caller, stages them in a versioned DER Builder payload before
  opening the same application URL in a new tab.
- The result tab consumes and removes the payload, removes the transfer query
  from its URL, mounts an editable viewer, and loads the bytes through the
  published `loadBytes()` method.
- A blocked popup or unavailable storage does not turn valid DER generation
  into a failed build; it is reported through UI status and the API Log.
- The same page continues to detect DerEditor's public Send to transfer query
  (`subtree` or `expand`) and makes that viewer editable.
- Do not implement a custom `viewer.html` or depend on DerEditor's internal
  local-storage key and payload formats.
- Use the published `pkistudio.ico` in normal tabs and `dereditor.ico` in
  result and transfer tabs.
- Simplify or hide the DER Builder shell in result and transfer modes so
  DerEditor owns the visible document workspace.
...
- generated DER opened directly in a new editable DerEditor tab;
- editable standalone view through DerEditor Send to from that result tab;
...
- versioned one-shot transfer for opening generated DER in a new editable tab;
- editable same-page result and DerEditor Send to transfer modes; and
- complete removal of PkiStudioJS and the custom viewer page.

Exit criteria:

- DerEditor Core round-trips every fixture.
- Save/edit is enabled in generated-result and Send to transfer viewers.
- Popup blocking is reported without discarding the successful build result or
  leaving staged payloads behind.
...
| `src/viewer.ts`, `viewer.html` | Remove and replace with same-`index.html` DER Builder result transfer and DerEditor Send to modes |
...
6. The final pane layout and new-tab result/transfer-mode wireframe.
...
- It uses only an exactly pinned, published DerEditor package through package
  exports.
- E2E verifies direct generated-result tabs, editable transfer viewers, and
  popup-blocking behavior.
